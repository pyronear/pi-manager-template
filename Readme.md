...
2. **Create Host Variables**:
   ...
   - A `main.yml` file for non-sensitive variables. Container the credentials.json file (the token will be generated by ansible)
   - A `main.vault.yml` file containing sensitive data.  Store the Linux user password (encrypted).
  ...
  If the Raspberry Pi connects to a WiFi network, add the WiFi credentials to the `main.vault.yml` file (by default, engines )

  You will have to encrypt the `main.vault.yml` file with your vault_passwrd, use:
  ```bash
    EDITOR=nano ansible-vault encrypt host_vars/<hostname>/main.vault.yml
  ```

  if you want to modify it afterwards, use :
  ```bash
    EDITOR=nano ansible-vault edit host_vars/<hostname>/main.vault.yml
  ```
