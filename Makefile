...
down: copy-inventory
	@bash -c 'set -a; source init_script/.env; set +a; ansible-playbook -i inventory/inventory playbooks/down-engines.yml -l mateo_local --vault-password-file=.vault_passwrd'

up: copy-inventory
	@bash -c 'set -a; source init_script/.env; set +a; ansible-playbook -i inventory/inventory playbooks/up-engines.yml -l mateo_local --vault-password-file=.vault_passwrd'
